...
2. Being there, clone this repository with `git@github.com:MaximilianPfitzenmaier/[repo-name].git [name]`.

### Build assets

1. Move to your app folder with `cd [name]/`
2. Install all npm dependendies by typing `npm install` or just `npm i`
3. Run `npm run build` to build all assets (e.g. styles)

### Run React app

1. Run `npm run dev` to start developer mode and visit the page at `localhost:5173`
